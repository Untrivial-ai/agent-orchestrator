...
func (m *Manager) aoSkillPointer() string {
	...
	return "\n\n" + "## Using the ao CLI\n\n" +
		"When using `ao`, read `" + skillFile + "` and only the relevant file under `" + commandsGlob + "`; do not load unrelated command guides. High priority: AO browser, preview, session spawning, control, and communication must use `ao`, never sibling supervisor CLIs, skills, or MCPs (Superset, Wmux, or equivalents). Unrelated tools remain allowed.\n\n" +
