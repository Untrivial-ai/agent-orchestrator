...
func TestResumeChatSession_PassesStoredConversationIDWhenProbeErrors(t *testing.T) {
	launcher := &recordingLauncher{startErr: ports.ErrChatResumeFailed}
	probe := &chatProbeAgent{err: errors.New("codex rollout probe failed")}
	mgr, store, _ := newChatManagerWithAgents(launcher, chatProbeAgents{agent: probe})
	seedChatResumeSession(store, domain.ActivityExited)
	rec := store.sessions["mer-1"]
	rec.Metadata.ProviderConversationID = "thread-error-probe"
	store.sessions["mer-1"] = rec

	_, err := mgr.ResumeAgentWithMode(context.Background(), "mer-1")
	if !errors.Is(err, ports.ErrChatResumeFailed) {
		t.Fatalf("err = %v, want ports.ErrChatResumeFailed", err)
	}
	if len(probe.probed) != 1 || probe.probed[0] != "thread-error-probe" {
		t.Fatalf("probed = %v, want [thread-error-probe]", probe.probed)
	}
	if len(launcher.started) != 1 {
		t.Fatalf("started %d chat controllers, want 1", len(launcher.started))
	}
	if got := launcher.started[0].ProviderConversationID; got != "thread-error-probe" {
		t.Fatalf("provider conversation id passed to StartChat = %q, want thread-error-probe", got)
	}
}
