...
func TestResumeExitedChatSessionDoesNotRequireTerminalRuntimeHandle(t *testing.T) {
	...
	if launcher.started[0].QueueRecoveryPolicy != domain.ChatQueueRecoveryNormal {
		t.Fatal("generic Resume agent authorized retained chat delivery outside verified auth recovery")
	}
...
func TestRestoreResumesChatRatherThanRelaunchingATerminal(t *testing.T) {
	...
	if resumed.QueueRecoveryPolicy != domain.ChatQueueRecoveryNormal {
		t.Fatal("ordinary restore authorized retained chat delivery without verified auth recovery")
	}
