...
func TestSpawnAndRestore_PrependsResolvedBinaryAndNodeDirsToRuntimePATH(t *testing.T) {
	...
	usrBin := filepath.Join(t.TempDir(), "usr", "bin")
	want := strings.Join([]string{filepath.Dir(daemonExe), binDir, nodeDir, usrBin}, string(os.PathListSeparator))
	if runtime.GOOS == "windows" {
		want = strings.Join([]string{filepath.Dir(daemonExe), binDir, usrBin}, string(os.PathListSeparator))
	}
...
func TestSpawn_DoesNotAddNodeRuntimeForNativeBinary(t *testing.T) {
	...
	want := strings.Join([]string{aoBin, binDir, usrBin}, string(os.PathListSeparator))
	if got := rt.lastCfg.Env["PATH"]; got != want {
		t.Fatalf("runtime env PATH = %q, want %q", got, want)
	}
}
