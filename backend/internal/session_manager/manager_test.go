...
func TestSpawn_ResolvesProjectConfig(t *testing.T) {
	...
	st.projects["mer"] = domain.ProjectRecord{ID: "mer", Config: domain.ProjectConfig{
		...
		Worker: domain.RoleOverride{Harness: domain.HarnessOpenCode, AgentConfig: domain.AgentConfig{Model: "worker-model"}},
	}}
	...
	if _, _, _, err := m.Spawn(ctx, ports.SpawnConfig{ProjectID: "bare", Kind: domain.KindWorker, Harness: domain.HarnessOpenCode}); err != nil {
		t.Fatal(err)
	}
...
func TestSpawnGatesResolvedProjectDefaultHarness(t *testing.T) {
	...
	if gate.harness != domain.HarnessOpenCode {
...
// TestSpawnModelValidation asserts spawn passes a model through to a harness that
// accepts arbitrary model ids. opencode is the only shipped harness and uses
// direct custom model entry, so AO owns no fixed list to reject against.
func TestSpawnModelValidation(t *testing.T) {
	...
	if _, _, _, err := m.Spawn(ctx, ports.SpawnConfig{ProjectID: "mer", Kind: domain.KindWorker, AgentConfig: ports.AgentConfig{Model: "custom-snapshot-id"}}); err != nil {
		t.Fatalf("spawn with custom model failed: %v", err)
	}
	if agent.lastConfig.Model != "custom-snapshot-id" {
		t.Fatalf("launch model = %q, want custom-snapshot-id", agent.lastConfig.Model)
	}
	if agent.lastConfig.Mode != "" {
		t.Fatalf("launch mode = %q, want empty for a model-based harness", agent.lastConfig.Mode)
	}
}
...
func TestSpawnModelPersisted(t *testing.T) {
	...
	st.projects["mer"] = domain.ProjectRecord{ID: "mer", Config: domain.ProjectConfig{
		...
		Worker:      domain.RoleOverride{Harness: domain.HarnessOpenCode, AgentConfig: domain.AgentConfig{Model: "role-model"}},
	}}
	agent := &recordingAgent{}
	rt := &fakeRuntime{}
	ws := &fakeWorkspace{}
...
func TestSpawn_ExplicitHarnessWinsWithoutProjectRoleHarness(t *testing.T) {
	...
	if _, _, _, err := m.Spawn(ctx, ports.SpawnConfig{ProjectID: "mer", Kind: domain.KindWorker, Harness: domain.HarnessOpenCode}); err != nil {
		t.Fatal(err)
	}
	if got := st.sessions["mer-1"].Harness; got != domain.HarnessOpenCode {
		t.Fatalf("explicit harness = %q, want %q", got, domain.HarnessOpenCode)
	}
}
...
func TestRestore_AgyAndCopilotWithoutAgentSessionIDFallBackToSavedPrompt(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "agy", harness: domain.HarnessOpenCode},
		{name: "copilot", harness: domain.HarnessOpenCode},
	} {
		t.Run(tc.name, func(t *testing.T) {
			st := newFakeStore()
			st.sessions["mer-1"] = domain.SessionRecord{
...
func TestValidateSpawnModelDefersToTheAgent(t *testing.T) {
	// opencode is the only harness AO ships, and its model surface is
	// agent-owned: AO accepts whatever the adapter reports rather than rejecting
	// a value against a static list.
	if err := validateSpawnModel(domain.HarnessOpenCode, "agent-owned-model"); err != nil {
		t.Fatalf("opencode dynamic model should be validated by the agent: %v", err)
	}
	if err := validateSpawnModel(domain.AgentHarness("unknown"), "anything"); err != nil {
		t.Fatalf("unknown harness model should be validated by the agent: %v", err)
	}
}
...
func TestSaveAndTeardownAll_QuiescesNativeAgentBeforeCapturingWork(t *testing.T) {
	...
	st.sessions["mer-1"] = domain.SessionRecord{
		ID: "mer-1", ProjectID: "mer", Kind: domain.KindWorker, Harness: domain.HarnessOpenCode,
...
func TestReconcileLive_RelaunchFailureLeavesSessionExitedAndRecoverable(t *testing.T) {
	...
	rec := domain.SessionRecord{
		ID: "s1", ProjectID: "p1", Harness: domain.HarnessOpenCode,
		Metadata: domain.SessionMetadata{
...
func TestReconcile_LivePassUsesConfiguredConcurrency(t *testing.T) {
	...
	for _, id := range ids {
		st.sessions[id] = domain.SessionRecord{
			ID: id, ProjectID: "p1", Harness: domain.HarnessOpenCode,
			Metadata: domain.SessionMetadata{
...
func TestReconcileLive_InconclusiveChatRecoveryDoesNotTeardown(t *testing.T) {
	...
	rec := domain.SessionRecord{
		ID: "chat-live", ProjectID: "p1", Harness: domain.HarnessOpenCode,
...
func TestReconcileLive_InconclusiveRuntimeProbeDoesNotRelaunch(t *testing.T) {
	...
	rec := domain.SessionRecord{
		ID: "s-inconclusive", ProjectID: "p1", Harness: domain.HarnessOpenCode,
		Metadata: domain.SessionMetadata{
...
func TestReconcile_AdoptAcrossDaemonRestart(t *testing.T) {
	...
	st.sessions["mer-1"] = domain.SessionRecord{
		ID: "mer-1", ProjectID: "mer", Kind: domain.KindOrchestrator, Harness: domain.HarnessOpenCode,
	...
	st.sessions["mer-2"] = domain.SessionRecord{
		ID: "mer-2", ProjectID: "mer", Kind: domain.KindWorker, Harness: domain.HarnessOpenCode,
	...
	st.sessions["mer-3"] = domain.SessionRecord{
		ID: "mer-3", ProjectID: "mer", Kind: domain.KindWorker, Harness: domain.HarnessOpenCode,
	...
	st.sessions["mer-4"] = domain.SessionRecord{
		ID: "mer-4", ProjectID: "mer", Kind: domain.KindWorker, Harness: domain.HarnessOpenCode,
...
func TestSend_PaneFallbackCannotPairLostPromptHookWithPriorTrustedAssistant(t *testing.T) {
	...
	// opencode has no provider turn identity, so canonical hooks with matching
	// owner provenance are the strongest evidence available: the later prompt
	// establishes the boundary and its Stop certifies the answer.
	if promoted.LatestUserPrompt != "later canonical prompt" ||
		...
		t.Fatalf("canonical hooks did not certify completion: %+v", promoted)
	}
}
...
func TestSend_PaneFallbackWinsAgainstStaleLifecycleProjection(t *testing.T) {
	...
	if promoted.Metadata.LatestUserPrompt != "later canonical prompt" ||
		...
		t.Fatalf("canonical hooks did not certify completion: %+v", promoted.Metadata)
	}
}
