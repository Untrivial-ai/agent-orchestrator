...
func (s *Service) EditMessage(
	...
	replacement := newController(id, source.owner(), conversation, generation, source.harness, provider, s.store, s.activity, s.log, s.newID, s.now, s.onAccountChanged, s.onCodexCapacityChanged, s.scheduleCardRefresh)
...
func (s *Service) restoreClosedSourceController(
	...
	replacement := newController(
		id, source.owner(), conversation, generation, source.harness, provider, s.store, s.activity, s.log, s.newID, s.now, s.onAccountChanged, s.onCodexCapacityChanged, s.scheduleCardRefresh)
