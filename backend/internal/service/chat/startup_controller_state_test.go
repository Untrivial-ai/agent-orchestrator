...
// on every spawn for the several seconds the conversation row takes to appear.
// See https://github.com/Untrivial-ai/agent-orchestrator/issues/5131.

// newStartupService builds a Service over a real store with no controller ever
// started, which is exactly the state a session is in between its durable row
// and its first conversation row. PageReader is intentionally left unset so
// SnapshotPage falls through to Snapshot, matching the non-production wiring.
func newStartupService(t *testing.T) (*chatsvc.Service, *sqlite.Store) {
	t.Helper()
	st := openStore(t)
...
// seedStartupSession writes a fresh chat session alongside the one openStore
// seeds, so a test can shape termination and activity without disturbing the
// shared row.
func seedStartupSession(t *testing.T, st *sqlite.Store, mutate func(*domain.SessionRecord)) domain.SessionID {
	t.Helper()
	now := time.Now().UTC()
...
func TestSnapshotReportsConnectingWhileControllerStartsUp(t *testing.T) {
	svc, st := newStartupService(t)
	id := seedStartupSession(t, st, nil)
...
func TestSnapshotPageReportsConnectingWhileControllerStartsUp(t *testing.T) {
	svc, st := newStartupService(t)
	id := seedStartupSession(t, st, nil)
...
func TestSnapshotStillReportsStoppedForSessionsThatReallyEnded(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			svc, st := newStartupService(t)
			id := seedStartupSession(t, st, tc.mutate)
...
func TestSnapshotReportsStoppedOnceAConversationExistsWithoutAController(t *testing.T) {
	...
	if _, err := st.CreateConversation(
		context.Background(), "conv-startup-1", domain.ConversationScopeSession,
