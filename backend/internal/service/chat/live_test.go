...
func TestLiveTextContinuesWhileProjectionIsBlocked(t *testing.T) {
	...
	h.conv.emit(ports.ChatEvent{Kind: ports.ChatEventMessageDelta, ProviderTurnID: turn.ProviderTurnID, ProviderItemID: "reply", Delta: "first"})
	select {
	...
	for _, text := range []string{" second", " third"} {
		h.conv.emit(ports.ChatEvent{Kind: ports.ChatEventMessageDelta, ProviderTurnID: turn.ProviderTurnID, ProviderItemID: "reply", Delta: text})
	}
