...
func (s *Service) ListPRSummaries(ctx context.Context, id domain.SessionID) ([]PRSummary, error) {
	...
	for _, group := range groups {
		...
		out = append(out, summarizePR(group.primary, checks, reviews, threads, comments, threadsExact, runs))
	}
	sortPRSummaries(out)
	return out, nil
}

func summarizePR(pr domain.PullRequest, checks []domain.PullRequestCheck, reviews []domain.PullRequestReview, threads []domain.PullRequestReviewThread, comments []domain.PullRequestComment, threadsExact bool, runs []domain.ReviewRun) PRSummary {
	return PRSummary{
		URL:              pr.URL,
