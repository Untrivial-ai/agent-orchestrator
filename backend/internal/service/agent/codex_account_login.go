...
// monitorLoginTerminal makes login completion a daemon-owned workflow instead
// of relying on a renderer terminal-exit notification. Windows ConPTY hosts
// deliberately survive their child to preserve scrollback, and a renderer can
// miss the one live exit frame while reconnecting. The child-status probe is
// durable across that race and starts verification as soon as the command has
// conclusively exited.
