...
// A skipped automatic pass and feedback that reached nobody are the two
// outcomes that look like success from the outside. Both have to arrive named,
// with their reason intact.
func TestSkippedAndWithheldReviewsReachTheWireWithTheirReason(t *testing.T) {
	sink, rec, closeSink := newWireSink(t)
	...
	policyOff := false
	withheldStore := &fakeStore{
		ok: true,
		run: domain.ReviewRun{
			ID: "run-1", SessionID: "worker-1", Status: domain.ReviewRunRunning,
			Harness: "codex", CreatedAt: time.Now().UTC(),
			PRURL: "https://github.com/acme/secret-repo/pull/7",
		},
		sessionAutoInjectReview: &policyOff,
	}
	withheldSvc := New(nil, withheldStore, WithTelemetry(sink), WithLifecycleReducer(&fakeReducer{}))
	if _, err := withheldSvc.Submit(context.Background(), "worker-1", "run-1",
		domain.VerdictChangesRequested, "fix the auth check", ""); err != nil {
		t.Fatalf("Submit: %v", err)
	}
	closeSink()

	skipped, ok := rec.find("ao.review.trigger_skipped")
	if !ok {
		t.Fatalf("ao.review.trigger_skipped never reached the wire; got %v", rec.names())
	}
	if skipped.props["reason"] != "worker_active" || skipped.props["trigger"] != "auto" {
		t.Fatalf("trigger_skipped props = %#v, want reason=worker_active trigger=auto", skipped.props)
	}

	withheld, ok := rec.find("ao.review.feedback_withheld")
	if !ok {
		t.Fatalf("ao.review.feedback_withheld never reached the wire; got %v", rec.names())
	}
	if withheld.props["reason"] != "auto_inject_off" {
		t.Fatalf("feedback_withheld.reason = %#v, want auto_inject_off", withheld.props["reason"])
	}
}
