...
func shouldEmitCLIInvocation(cmd *cobra.Command) bool {
	...
	case "ao daemon", "ao start", "ao completion", "ao help", "ao pty-host", "ao chat-host", "ao codex-login", "ao claude-login", "ao agent-process", "ao agent-process supervise", "ao mcp":
		return false
	default:
		return true
	}
