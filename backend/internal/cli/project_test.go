...
func TestProjectSetConfigConcurrencyCapFlag(t *testing.T) {
	cfg := setConfigEnv(t)
	srv, capture := projectServer(t, http.StatusOK, `{"project":{"id":"demo","path":"/repo/demo"}}`)
	writeRunFileFor(t, cfg, srv)

	_, errOut, err := executeCLI(t, Deps{ProcessAlive: func(int) bool { return true }},
		"project", "set-config", "demo", "--max-concurrent-sessions", "3")
	if err != nil {
		t.Fatalf("unexpected error: %v\nstderr=%s", err, errOut)
	}
	var got setConfigRequest
	...
	if got.Config.MaxConcurrentSessions != 3 {
		t.Fatalf("MaxConcurrentSessions = %d, want 3", got.Config.MaxConcurrentSessions)
	}
}
