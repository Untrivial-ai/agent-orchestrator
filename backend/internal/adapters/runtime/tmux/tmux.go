...
func (r *Runtime) Create(ctx context.Context, cfg ports.RuntimeConfig) (ports.RuntimeHandle, error) {
	...
	args := newSessionArgs(id, cfg.WorkspacePath, launcherShell(r.shell), launchCmd)
...
func (r *Runtime) Restart(ctx context.Context, handle ports.RuntimeHandle, cfg ports.RuntimeConfig) (ports.RuntimeHandle, error) {
	...
	launchCmd := buildLaunchCommand(cfg)
	if _, err := r.runForSession(ctx, id, respawnPaneArgs(id, cfg.WorkspacePath, launcherShell(r.shell), launchCmd)...); err != nil {
...
// launcherShell returns a POSIX-compatible shell (/bin/sh or /bin/bash) to
// execute launch scripts generated by buildLaunchCommand in POSIX syntax. When
// $SHELL is a non-POSIX shell (such as fish, nushell, or tcsh), launcherShell
// falls back to /bin/sh so prelude env assignments and exports execute cleanly
// before transitioning to the interactive shell.
func launcherShell(shellPath string) string {
	if shellPath == "" {
		return "/bin/sh"
	}
	base := strings.ToLower(filepath.Base(shellPath))
	switch base {
	case "fish", "nu", "nushell", "csh", "tcsh", "elvish", "xonsh":
		return "/bin/sh"
	}
	return shellPath
}
