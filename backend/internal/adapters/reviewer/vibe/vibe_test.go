...
func TestReviewCommandLaunchesHostTrustedPlanTUIWithAutoApproval(t *testing.T) {
	inv := invocation(t)
	...
	want := []string{"/opt/vibe/bin/vibe", "--trust", "--workdir", inv.WorkspacePath, "--add-dir", inv.TaskPromptRoot, "--agent", "plan", "--auto-approve", inv.Prompt}
