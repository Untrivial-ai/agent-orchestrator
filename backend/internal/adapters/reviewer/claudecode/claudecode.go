...
// review, and `ao review submit` to record the verdict). Claude Code ≥ 2.1.257
// still prompts on Bash commands its analyzer cannot verify statically even
// when an allow rule matches; the PermissionRequest hook AO installs answers
// those for reviewers (see cli.reviewerPermissionDecision) so the pane never
// stalls.
