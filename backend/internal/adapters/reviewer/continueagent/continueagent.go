...
// HostTrustWarning documents that Continue auto mode bypasses approval prompts.
const HostTrustWarning = "experimental host-trusted reviewer: Continue auto mode bypasses approval prompts and is not OS isolation; it can be changed from the interactive TUI"
...
// ReviewCommand launches Continue in auto mode, so reviews are not blocked by
// native approval prompts, and injects the task after readiness. CLI discovery
// and state are redirected to an AO-owned profile.
