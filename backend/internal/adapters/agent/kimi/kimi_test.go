...
func TestGetAgentHooksSeedsAOManagedConfigFromKeyringOAuthUserKimiHome(t *testing.T) {
	workspace := t.TempDir()
	userHome := t.TempDir()
	aoHome := t.TempDir()
	...
	if _, err := os.Stat(filepath.Join(aoHome, "credentials", "kimi-code.json")); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("keyring profile created AO credentials file: %v", err)
	}
	source, err := os.ReadFile(filepath.Join(userHome, "config.toml"))
	if err != nil {
		t.Fatalf("read source config: %v", err)
...
func TestGetAgentHooksSeedsCurrentJSONConfigBeforeCredentialFallback(t *testing.T) {
	workspace := t.TempDir()
	userHome := t.TempDir()
	aoHome := t.TempDir()
  ...
  "models": {
	...
	credentials := []byte(`{"refresh_token":"user-refresh"}`)
	if err := os.WriteFile(filepath.Join(credentialsDir, "kimi-code.json"), credentials, 0o600); err != nil {
		t.Fatal(err)
	}
	...
	seededCredentials, err := os.ReadFile(filepath.Join(aoHome, "credentials", "kimi-code.json"))
	if err != nil {
		t.Fatalf("read AO credentials: %v", err)
	}
	if string(seededCredentials) != string(credentials) {
		t.Fatalf("AO credentials = %s, want %s", seededCredentials, credentials)
	}
}
...
func TestGetAgentHooksSeedsAOManagedConfigFromMalformedKeyringCredentials(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(aoHome, "credentials", "kimi-code.json")); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("malformed keyring profile copied AO credentials: %v", err)
	}
}
