...
// writeCodexRollout persists a rollout whose leading session_meta optionally
// names the thread it was forked from, matching what Codex writes on disk.
func writeCodexRollout(t *testing.T, codexHome, id, forkedFrom string) {
	t.Helper()
	dir := filepath.Join(codexHome, "sessions", "2026", "09", "10")
	if err := os.MkdirAll(dir, 0o755); err != nil {
		t.Fatal(err)
	}
	payload := map[string]any{"id": id, "session_id": id}
	if forkedFrom != "" {
		payload["forked_from_id"] = forkedFrom
		payload["forked_from_ordinal_exclusive"] = 42
	}
	line, err := json.Marshal(map[string]any{
		"timestamp": "2026-09-10T21:09:33.775Z", "type": "session_meta", "payload": payload,
	})
	if err != nil {
		t.Fatal(err)
	}
	body := append(line, '\n')
	body = append(body, []byte(`{"type":"event_msg","payload":{}}`+"\n")...)
	path := filepath.Join(dir, "rollout-2026-09-10T14-09-33-"+id+".jsonl")
	if err := os.WriteFile(path, body, 0o600); err != nil {
		t.Fatal(err)
	}
}

// A Chat -> TUI switch leaves Codex resuming the thread in its TUI, and Codex
// forks rather than writing in place. Succession is what lets the later
// TUI -> Chat handoff recognize the descendant as the same conversation.
func TestNativeConversationSucceedsFollowsCodexForkChain(t *testing.T) {
	const (
		root  = "01a077dc-1de6-7c81-af05-c88acfb7f1d5"
		mid   = "01a08d27-878b-7db0-9e84-9a921099f4a8"
		leaf  = "01a09000-1111-7222-8333-444455556666"
		alien = "01a0aaaa-2222-7333-8444-555566667777"
	)
	p := &Plugin{}
	codexHome := t.TempDir()
	t.Setenv("CODEX_HOME", t.TempDir())
	env := map[string]string{"CODEX_HOME": codexHome}
	writeCodexRollout(t, codexHome, root, "")
	writeCodexRollout(t, codexHome, mid, root)
	writeCodexRollout(t, codexHome, leaf, mid)
	writeCodexRollout(t, codexHome, alien, "")

	for _, tc := range []struct {
		name                   string
		candidate, predecessor string
		want                   bool
	}{
		{"direct fork continues its parent", mid, root, true},
		{"multi-hop fork chain continues its root", leaf, root, true},
		{"unrelated thread does not continue", alien, root, false},
		{"ancestor does not continue its own descendant", root, mid, false},
		{"identical handles are an ordinary resume", root, root, false},
		{"unknown candidate proves nothing", "01a0bbbb-3333-7444-8555-666677778888", root, false},
		{"unknown predecessor proves nothing", leaf, "01a0cccc-4444-7555-8666-777788889999", false},
		{"non-uuid handles prove nothing", "not-a-uuid", root, false},
	} {
		t.Run(tc.name, func(t *testing.T) {
			got, err := p.NativeConversationSucceeds(
				context.Background(), ports.SessionRef{}, tc.candidate, tc.predecessor, env)
			if err != nil {
				t.Fatalf("NativeConversationSucceeds: %v", err)
			}
			if got != tc.want {
				t.Fatalf("succeeds(%s -> %s) = %v, want %v", tc.candidate, tc.predecessor, got, tc.want)
			}
		})
	}
}

// A rollout set AO cannot read is not evidence of succession. Every one of these
// must fail closed so the caller keeps refusing the mismatched handle.
func TestNativeConversationSucceedsFailsClosedOnUnreadableRollouts(t *testing.T) {
	const (
		root  = "01a077dc-1de6-7c81-af05-c88acfb7f1d5"
		child = "01a08d27-878b-7db0-9e84-9a921099f4a8"
	)
	p := &Plugin{}

	t.Run("cycle terminates without claiming succession", func(t *testing.T) {
		codexHome := t.TempDir()
		t.Setenv("CODEX_HOME", t.TempDir())
		writeCodexRollout(t, codexHome, child, root)
		writeCodexRollout(t, codexHome, root, child)
		got, err := p.NativeConversationSucceeds(context.Background(), ports.SessionRef{},
			child, "01a0dddd-5555-7666-8777-888899990000", map[string]string{"CODEX_HOME": codexHome})
		if err != nil || got {
			t.Fatalf("cycle: succeeds=%v err=%v", got, err)
		}
	})

	t.Run("malformed leading record proves nothing", func(t *testing.T) {
		codexHome := t.TempDir()
		t.Setenv("CODEX_HOME", t.TempDir())
		dir := filepath.Join(codexHome, "sessions", "2026", "09", "10")
		if err := os.MkdirAll(dir, 0o755); err != nil {
		...
		path := filepath.Join(dir, "rollout-2026-09-10T14-09-33-"+child+".jsonl")
		if err := os.WriteFile(path, []byte("not json\n"), 0o600); err != nil {
			t.Fatal(err)
		}
		writeCodexRollout(t, codexHome, root, "")
		got, err := p.NativeConversationSucceeds(context.Background(), ports.SessionRef{},
			child, root, map[string]string{"CODEX_HOME": codexHome})
		if err != nil || got {
			t.Fatalf("malformed: succeeds=%v err=%v", got, err)
		}
	})

	t.Run("compressed rollout is not parsed", func(t *testing.T) {
		codexHome := t.TempDir()
		t.Setenv("CODEX_HOME", t.TempDir())
		dir := filepath.Join(codexHome, "sessions", "2026", "09", "10")
		if err := os.MkdirAll(dir, 0o755); err != nil {
		...
		path := filepath.Join(dir, "rollout-2026-09-10T14-09-33-"+child+".jsonl.zst")
		...
		got, err := p.NativeConversationSucceeds(context.Background(), ports.SessionRef{},
			child, root, map[string]string{"CODEX_HOME": codexHome})
		if err != nil || got {
			t.Fatalf("compressed: succeeds=%v err=%v", got, err)
		}
	})
}
