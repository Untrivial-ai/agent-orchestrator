...
func TestGetLaunchCommandBuildsCrossPlatformArgv(t *testing.T) {
	...
	want = append(want,
		...
		"-c", "model_instructions_file="+codexTOMLConfigString(systemFile),
		"--", "-fix this",
	)
...
func TestGetLaunchCommandUsesSystemPromptFile(t *testing.T) {
	...
	if !containsSubsequence(cmd, []string{"-c", "model_instructions_file=" + codexTOMLConfigString(promptFile)}) {
...
func TestGetRestoreCommandUsesSystemPromptFile(t *testing.T) {
	...
	if !containsSubsequence(cmd, []string{"-c", "model_instructions_file=" + codexTOMLConfigString(promptFile), "thread-123"}) {
