...
func TestQwenMalformedConfigDoesNotExposeLowerPriorityCredentials(t *testing.T) {
	for _, tc := range []struct{ name, path, content string }{
		...
		{"null selected auth type", "workspace/.qwen/settings.json", `{"security":{"auth":{"selectedType":null}}}`},
		{"null selected model", "workspace/.qwen/settings.json", `{"model":{"name":null}}`},
		{"null auth container", "workspace/.qwen/settings.json", `{"security":{"auth":null}}`},
		{"null security container", "workspace/.qwen/settings.json", `{"security":null}`},
		{"null model container", "workspace/.qwen/settings.json", `{"model":null}`},
		{"null API key", "workspace/.qwen/settings.json", `{"security":{"auth":{"apiKey":null}}}`},
		{"null base URL", "workspace/.qwen/settings.json", `{"security":{"auth":{"baseUrl":null}}}`},
		{"null providers container", "workspace/.qwen/settings.json", `{"modelProviders":null}`},
		{"null provider models", "workspace/.qwen/settings.json", `{"modelProviders":{"gemini":null}}`},
		{"null provider envKey", "workspace/.qwen/settings.json", `{"modelProviders":{"gemini":[{"id":"gemini","envKey":null}]}}`},
		{"null protocol container", "workspace/.qwen/settings.json", `{"providerProtocol":null}`},
		{"null env container", "workspace/.qwen/settings.json", `{"env":null}`},
