...
func TestVersionGate(t *testing.T) {
	for _, tc := range []struct {
		value string
		ok    bool
	}{{"1.1.54", true}, {"qoder 1.2.0", true}, {"1.1.53", false}, {"unknown", false}} {
		if err := validateVersionOutput(tc.value); (err == nil) != tc.ok {
			t.Errorf("validateVersionOutput(%q) = %v", tc.value, err)
		}
	}
}
