...
const detachPR = `-- name: DetachPR :execrows
DELETE FROM pr
WHERE url = ? AND session_id = ? AND attachment_source = ?
`

type DetachPRParams struct {
	URL              string
	SessionID        domain.SessionID
	AttachmentSource string
}

func (q *Queries) DetachPR(ctx context.Context, arg DetachPRParams) (int64, error) {
	result, err := q.db.ExecContext(ctx, detachPR, arg.URL, arg.SessionID, arg.AttachmentSource)
	if err != nil {
		return 0, err
	}
...
SELECT url, session_id, number, pr_state, review_decision, ci_state, mergeability, updated_at, provider, host, repo, source_branch, target_branch, head_sha, title, additions, deletions, changed_files, author, base_sha, merge_commit_sha, is_draft, is_merged, is_closed, provider_state, provider_mergeable, provider_merge_state_status, html_url, created_at_provider, updated_at_provider, merged_at_provider, closed_at_provider, metadata_hash, ci_hash, review_hash, observed_at, ci_observed_at, review_observed_at, last_nudge_signature, state_changed_at, auto_inject_ci, provider_id, attachment_source
FROM pr
WHERE provider = ?1
  AND host = ?2
  AND provider_id = ?3
...
func (q *Queries) GetPRByURLOrAlias(ctx context.Context, url string) (PR, error) {
	...
	err := row.Scan(
		...
		&i.StateChangedAt,
		&i.AutoInjectCI,
		&i.ProviderID, &i.AttachmentSource)
	return i, err
}
...
SELECT url, session_id, number, pr_state, review_decision, ci_state, mergeability, updated_at, provider, host, repo, source_branch, target_branch, head_sha, title, additions, deletions, changed_files, author, base_sha, merge_commit_sha, is_draft, is_merged, is_closed, provider_state, provider_mergeable, provider_merge_state_status, html_url, created_at_provider, updated_at_provider, merged_at_provider, closed_at_provider, metadata_hash, ci_hash, review_hash, observed_at, ci_observed_at, review_observed_at, last_nudge_signature, state_changed_at, auto_inject_ci, provider_id, attachment_source FROM pr
WHERE pr.session_id = ?ORDER BY updated_at DESC
`
...
func (q *Queries) ListPRsBySession(ctx context.Context, sessionID domain.SessionID) ([]PR, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&i.StateChangedAt,
			&i.AutoInjectCI,
			&i.ProviderID, &i.AttachmentSource); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
...
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,
    COALESCE((SELECT auto_inject_ci FROM sessions WHERE id = ?), TRUE), ?)ON CONFLICT (url) DO UPDATE SET
    number = excluded.number,
    state_changed_at = CASE
