-- +goose Up
-- workflow_mode is a user-controlled delivery stage, not display status. New
-- sessions start in "planning" until a user pushes them into "building" (or the
-- orchestrator, running in build mode, spawns them straight into building).
-- The kanban board's Planning/Building lanes derive from this field.
-- +goose StatementBegin
ALTER TABLE sessions ADD COLUMN workflow_mode TEXT NOT NULL DEFAULT 'planning';
-- +goose StatementEnd
-- The CDC trigger already rebuilds the session invalidation stream for every
-- user-visible policy change; extend it so a workflow-mode change also
-- invalidates the stream (the board must re-derive lanes when a card moves
-- between Planning and Building). Keep every condition and payload field from
-- migration 0109 and add only workflow_mode.
-- +goose StatementBegin
...
-- +goose Down
-- Restore the pre-0150 trigger, then drop the column.
-- +goose StatementBegin
DROP TRIGGER IF EXISTS sessions_cdc_update;
-- +goose StatementEnd
-- +goose StatementBegin
...
-- +goose StatementEnd
-- +goose StatementBegin
ALTER TABLE sessions DROP COLUMN workflow_mode;
-- +goose StatementEnd
