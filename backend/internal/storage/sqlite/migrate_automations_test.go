...
// Parallel feature branches commonly burn goose version 156 without the
// automations schema. Open must release that ledger entry and apply
// 0156_automations.sql, or daemon boot dies on automation_run_id.
func TestMigrateRepairsBurnedAutomationsVersion(t *testing.T) {
	dataDir := t.TempDir()
	db := openMigratedDatabaseCopyAt(t, dataDir, 152, pragmas)
	if _, err := db.Exec(`INSERT INTO goose_db_version (version_id, is_applied) VALUES (156, 1)`); err != nil {
		t.Fatalf("burn version 156: %v", err)
	}
...
func TestMigration0156EnforcesAutomationRunIdentity(t *testing.T) {
...
func TestMigration0156LinksOneSessionAndPreservesItOnAutomationDelete(t *testing.T) {
	db := openMigratedDatabaseCopy(t, 156)
