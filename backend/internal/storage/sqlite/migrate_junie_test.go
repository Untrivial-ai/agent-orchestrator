...
func TestMigration0155AllowsJunieAndReversesBothHistoricalSchemas(t *testing.T) {
	for _, legacyQM := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			var version int
			if err := db.QueryRow(`SELECT MAX(version_id) FROM goose_db_version WHERE is_applied = 1`).Scan(&version); err != nil || version != 155 {
				t.Fatalf("migration version = %d, err = %v; want 155", version, err)
			}
