...
func (c *SessionsController) spawn(w http.ResponseWriter, r *http.Request) {
	...
	sess, promptBytes, systemPromptBytes, err := c.Svc.Spawn(r.Context(), ports.SpawnConfig{ProjectID: in.ProjectID, IssueID: in.IssueID, Kind: in.Kind, Harness: in.Harness, Branch: in.Branch, RequestedMode: in.Mode, Prompt: in.Prompt, Model: strings.TrimSpace(in.Model), DisplayName: displayName, Attachments: attachments})
	if err != nil {
		envelope.WriteError(w, r, err)
		return
	}
