...
- **The browser web UI extends what that same listener may serve — it adds no new bind** — see `docs/adr/0005-web-ui-on-the-lan-listener.md`. The LAN listener may also serve the **browser web UI**: an embedded SPA plus a self-contained `/login` page, with everything else still behind auth. Alongside `GET /api/v1/identity`, the unauthenticated surface is `GET /login` and `GET`/`POST /api/v1/web/session`, which carry their own lockout and credential handling; nothing else is exempt. It accepts two additional credentials alongside Bearer — a **web session cookie** and, when explicitly enabled, a **trusted tailnet identity** header injected by `tailscale serve`. Its bind host is configurable (`AO_CONNECT_BIND_HOST`, default `0.0.0.0` so existing behavior is unchanged). Three rules are load-bearing, each with tests: cookie/identity requests are **CSRF-checked**; cross-origin `/mux` upgrades are **rejected** under cookie or identity auth while Bearer keeps skip-verify for native clients; and **identity trust may only be enabled on a loopback bind** — the daemon refuses to start otherwise, because a header is spoofable by anything that can reach the socket.
