...
// the feed yml. We deliberately do NOT surface blockMapSize for the sidecars.
//
// What that means, per platform (verified against the published
...
//          reading the blockmap appended to the AppImage's own tail by the
//          maker (app-builder-lib appendBlockmap). feed.mjs publishes that
//          embedded blockMapSize, so the updater diffs against the running
//          AppImage and fetches only the changed chunks; when the yml omits it,
//          the attempt computes a NaN offset and falls back to the full
//          download (#5576). The sidecars we write for linux are read by nobody.
