...
For a headless host, run the [Docker daemon image](/docs/docker): mount a workspace, set a Connect Mobile password, and publish port `3011`. The image does not serve the desktop UI in a browser; use the mobile app or an API client against the LAN listener.

For unattended work, keep the host awake and reachable on its local network. AO does not turn the local desktop product into a hosted multi-user service.
