...
<PlatformSupport
	...
	linux="full"
	windows="full"
	note={<>Terminal UI runs on AO's native terminal host on every platform; tmux is only a legacy fallback on macOS/Linux. Chat uses native provider controllers.</>}
/>
...
Both Chat and Terminal UI are supported. New sessions run on AO's native terminal host, so tmux is not required. Install it only to reopen sessions created before that host existed:
...
Linux supports the desktop app, Chat, Terminal UI on AO's native terminal host, and the in-app browser inspector. tmux is only a legacy fallback; install it with your distribution package manager if you need it:
