...
`ao doctor` checks daemon/config state, the data directory and database, Git, and — as advisory information — tmux on macOS/Linux. It is not required for a normal desktop-only install.
...
<Accordions>
	...
	<Accordion title="A TUI session will not start on macOS or Linux">
		New sessions run on AO's native terminal host and do not need tmux. Check the agent harness first; install tmux only to reopen a session created before that host existed.
	</Accordion>
