...
├── mcp                            Run the AO MCP server (stdio or loopback HTTP)
...
Runs the AO MCP server so Claude Code, Cursor, or Codex can drive the local daemon. Default transport is stdio; `--http 127.0.0.1:8787` serves streamable HTTP on loopback only. See the [MCP server](/docs/mcp) page for client config snippets.
