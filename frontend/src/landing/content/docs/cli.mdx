...
| Flag | Purpose |
| --- | --- |
| `--name <name>` | Required sidebar name, at most 100 characters. |
...
References become stale after navigation, DOM replacement, frame changes, or tab changes. Take another snapshot when AO reports `STALE_REFERENCE`. `ao browser snapshot --delta` returns the full tree once, then only `unchanged` or the changes since the last snapshot the agent received; add `--full` to get the full tree again. `click` and `drag` accept `--human` for curved pointer movement on pages that ignore instant clicks, and `ao browser screenshot --annotate` numbers interactive elements in the image and lists their refs. Network capture is off by default, metadata-only, bounded, and automatically expires.
