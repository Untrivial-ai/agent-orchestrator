...
TUI sessions preserve the selected harness's native terminal experience. The desktop terminal connects through one daemon WebSocket mux backed by AO's native terminal host on every platform; tmux remains only for sessions started before that host existed.
