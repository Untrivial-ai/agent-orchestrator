...
You can configure multiple reviewers for cross-provider review workflows. Each configured reviewer will evaluate the same PR head independently. The PR is considered fully reviewed only when every configured reviewer has approved the current head. Any `changes_requested` result keeps the review unresolved. Reviewer failure or timeout is surfaced as incomplete, never treated as approval. Individual verdicts remain attributed and visible by reviewer.
