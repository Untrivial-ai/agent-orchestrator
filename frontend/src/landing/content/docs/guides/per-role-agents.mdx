...
- **Not every agent honors `agentConfig.model`.** Model override requires the underlying CLI to expose a model-selection flag or config key. Amp (`agent: amp`) currently has none — it only exposes an interactive mode switcher (`Ctrl+O` in the TUI), not a specific model string — so `agentConfig.model` is silently ignored for Amp sessions. Check the agent's [plugin docs](/docs/plugins/agents) if a model override doesn't seem to take effect.
