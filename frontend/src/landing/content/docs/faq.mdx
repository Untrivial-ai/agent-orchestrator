...
<Accordions expandAll>
	...
	<Accordion title="Does AO upload my source code?">
		AO coordinates local worktrees and invokes your installed agents and GitHub tooling. Those tools follow their own provider data policies. AO also sends usage events to PostHog in standard builds, including the repository owner and authenticated GitHub login associated with an installation (see [telemetry and privacy](https://useao.dev/privacy)); session recording is disabled by default, and local paths and URLs are redacted if a time-boxed investigation enables it.
	</Accordion>
