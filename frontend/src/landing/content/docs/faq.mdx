...
<Accordions expandAll>
	...
	<Accordion title="Which source-control systems are supported?">
		GitHub and GitLab are the shipped SCM observers in the current Go rewrite. Both supply pull/merge-request, CI, review, and mergeability facts, and both are polled side by side. GitLab covers self-managed instances once their hosts are allowlisted — see [GitLab SCM](/docs/plugins/scm/gitlab).
	</Accordion>

	<Accordion title="Does AO need webhooks?">
		No. The SCM observers poll with authentication, ETag guards, and semantic diffing. Live desktop updates come from the daemon's SQLite CDC event stream.
	</Accordion>
