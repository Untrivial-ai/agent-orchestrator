...
| tmux | Legacy Terminal UI sessions on macOS/Linux | New sessions run on AO's native terminal host and do not need it. Install it only to reopen sessions created before that host existed, or as a fallback runtime. |
...
<Accordions>
	...
	<Accordion title="Terminal UI cannot start on macOS or Linux">
		New sessions run on AO's native terminal host and do not need tmux. Check the agent harness first, and choose Chat when the selected harness supports it. Install `tmux` only to reopen a session created before that host existed.
	</Accordion>
