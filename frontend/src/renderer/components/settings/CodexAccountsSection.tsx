...
export function CodexAccountsSection({ titleHidden }: { titleHidden?: boolean }) {
	...
	const switchSourceAvailable = Boolean(deviceVerified && data?.activeAccountId && activeAccount && !data.unmanagedGlobalAccount);
	...
	useEffect(() => {
		const status = deviceReconciliation?.status;
		const previous = previousReconciliationStatus.current;
		previousReconciliationStatus.current = status;
		if (status === "verified" && previous && previous !== "verified") {
			setAnnouncement(t("settings.codexAccounts.reconciliationRecovered"));
		}
	}, [deviceReconciliation?.status, t]);
	...
	const summary = useMemo(() => {
		...
		if (!deviceVerified) {
			if (deviceReconciliation?.status === "checking") return t("settings.codexAccounts.reconciliationChecking");
			if (deviceReconciliation?.status === "temporarily_unavailable") return t("settings.codexAccounts.reconciliationRetrying");
			if (deviceReconciliation?.status === "blocked") return t("settings.codexAccounts.reconciliationBlocked");
		}
	...
	}, [accountsError, activeAccount, activeAuthentication?.key, data, deviceReconciliation?.status, deviceVerified, switchPresentation?.busy, switchStatus, t]);
	...
	return <SettingsSection title={t("settings.codexAccounts.title")} sectionId="codex-accounts" titleHidden={titleHidden}>
		<AgentProviderGroup provider="codex" name="Codex" summary={summary} expanded={providerExpanded || Boolean(activeLogin)} onExpandedChange={setProviderExpanded} collapseLocked={Boolean(activeLogin)} action={<div className="flex items-center gap-2">{switchPresentation?.busy && switchStatus ? <LoaderCircle className="size-5 animate-spin text-muted-foreground" aria-label={switchStatus} /> : null}{switchSourceAvailable && switchTargets.length > 0 ? <DropdownMenu><DropdownMenuTrigger asChild><Button type="button" size="sm" variant="outline" disabled={mutationDisabled || switchUnsupported} title={switchUnsupported && data ? t(codexAccountReasonKey(data.capabilities.globalSwitch.reasonCode)) : undefined}><ArrowRightLeft aria-hidden="true" />{t("settings.codexAccounts.switchConfirm")}</Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="min-w-64">{switchTargets.map((account) => { const authenticated = codexAuthenticationDisplay(account).key === "settings.codexAccounts.signedIn"; const targetSummary = [formatAuthMethod(account.authMethod), formatPlanName(account.capacity.plan)].filter(Boolean).join(" · "); return <DropdownMenuItem key={account.id} disabled={account.status !== "valid" || !authenticated} onSelect={() => openPending("switch", account)}><UserRound aria-hidden="true" /><div className="min-w-0"><p className="truncate text-foreground">{account.label}</p>{targetSummary ? <p className="truncate text-micro text-muted-foreground">{targetSummary}</p> : null}</div></DropdownMenuItem>; })}</DropdownMenuContent></DropdownMenu> : null}<Button type="button" size="sm" title={accountsError ?? undefined} onClick={() => void beginLogin()} disabled={mutationDisabled || data?.capabilities.nativeLogin.state !== "supported"}><Plus aria-hidden="true" />{t("settings.codexAccounts.add")}</Button></div>}>
			...
			{deviceReconciliation?.status === "checking" ? <p className="border-b border-border px-4 py-3 text-xs text-muted-foreground" role="status" aria-live="polite">{t("settings.codexAccounts.reconciliationChecking")}</p> : null}
			{deviceReconciliation?.status === "temporarily_unavailable" ? <p className="border-b border-border px-4 py-3 text-xs text-warning" role="status" aria-live="polite">{t("settings.codexAccounts.reconciliationRetrying")}</p> : null}
