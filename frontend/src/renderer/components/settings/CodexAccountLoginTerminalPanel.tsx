...
export function CodexAccountLoginTerminalPanel({ activeLogin, pending, revealed, onAttached, onCheckAgain, onRetry }: {
	activeLogin: CodexActiveLogin;
	pending: boolean;
	/** Parent-owned reveal (attach or the 5s preload fallback). Without this the
	 * panel stays h-0 forever when attach never arrives. */
	revealed: boolean;
	...
	// it reports attached (or the parent fallback fires): users only ever see
	// a working terminal, never a loader or an empty box. The tree shape never
	// changes, so revealing never remounts (or re-attaches) the terminal.
	...
	const ready = revealed || attachedKey === operationKey;
	useEffect(() => {
		if (!ready) return;
