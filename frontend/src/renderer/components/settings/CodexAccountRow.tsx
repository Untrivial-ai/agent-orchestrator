...
export function CodexAccountRow({ account, expanded, resetCreditSupported, mutationDisabled, deviceMutationDisabled, canUse, useBusy, resetBusy, authenticationRetryBusy, logoutBusy, deleteBusy, activeLogin, loginPending, loginRevealed, onToggle, onUseAccount, onUseReset, onRetryAuthentication, onSignIn, onLogout, onDelete, onCheckLogin, onAttachedLogin, onRetryLogin }: {
	...
	onDelete: () => void;
	onCheckLogin: () => void;
	onAttachedLogin: (operationKey: string) => void;
	onRetryLogin: () => void;
}) {
	...
	return (
		<div id={`codex-account-${account.id}`} data-account-id={account.id} tabIndex={-1} className="px-4 py-3 outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring">
			<div className={`flex justify-between gap-3 ${canUse ? "items-start" : "items-center"}`}>
				...
				{!needsSignIn && canUse ? <Button type="button" size="sm" variant="outline" aria-label={`${t("settings.codexAccounts.useAccount")} ${account.label}`} disabled={deviceMutationDisabled || useBusy} onClick={onUseAccount}>{useBusy ? <LoaderCircle className="animate-spin" aria-hidden="true" /> : null}{t("settings.codexAccounts.useAccountShort")}</Button> : null}
			...
			{activeLogin ? <div className={loginRevealed ? "ml-9 mt-4 pb-1" : "h-0 overflow-hidden"}><CodexAccountLoginTerminalPanel activeLogin={activeLogin} pending={loginPending} revealed={loginRevealed} onAttached={onAttachedLogin} onCheckAgain={onCheckLogin} onRetry={onRetryLogin} /></div> : null}
		</div>
	);
}
