...
export function CodexAccountRow({ account, expanded, resetCreditSupported, mutationDisabled, deviceMutationDisabled, canUse, useBusy, resetBusy, authenticationRetryBusy, logoutBusy, deleteBusy, activeLogin, loginPending, loginRevealed, onToggle, onUseAccount, onUseReset, onRetryAuthentication, onSignIn, onLogout, onDelete, onCheckLogin, onAttachedLogin, onRetryLogin }: {
	...
	return (
		<div id={`codex-account-${account.id}`} data-account-id={account.id} tabIndex={-1} className="px-4 py-3 outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring">
			...
			{activeLogin ? <div className={loginRevealed ? "ml-9 mt-4 pb-1" : "h-0 overflow-hidden"}><CodexAccountLoginTerminalPanel activeLogin={activeLogin} pending={loginPending} onAttached={onAttachedLogin} onCheckAgain={onCheckLogin} onRetry={onRetryLogin} /></div> : null}
		</div>
	);
}
