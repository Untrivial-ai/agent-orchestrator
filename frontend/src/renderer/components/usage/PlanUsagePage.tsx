				...
				<div className="mx-auto flex w-full max-w-5xl flex-col gap-6">
					<header>
						<div className="flex items-center gap-2.5">
							<Gauge aria-hidden="true" className="size-5 text-logo-accent" />
							<h1 className="text-xl font-semibold tracking-tight">{t("planUsage.title")}</h1>
						</div>
						<p className="mt-1.5 max-w-2xl text-sm text-muted-foreground">{t("planUsage.description")}</p>
					</header>
					...
					<p className="text-xs leading-relaxed text-muted-foreground">{t("planUsage.disclaimer")}</p>
				</div>
			</div>
		</CenterPanelShell>
	);
...
function ProviderQuotaCard({ quota }: { quota: ProviderQuota }) {
	...
	return (
		<Card className="border border-border bg-background shadow-sm ring-0">
			<CardHeader className="border-b border-border pb-4">
				<CardTitle className="flex items-center gap-2">
					<span>{title}</span>
					{quota.planType ? <span className="rounded-full bg-muted px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">{quota.planType}</span> : null}
				</CardTitle>
	...
	return (
		<div>
			<div className="mb-1.5 flex items-start justify-between gap-3">
				<div><p className="text-sm font-medium">{label}</p><p className="text-xs text-muted-foreground">{windowLabel(limit.windowDurationSeconds ?? undefined, limit.windowType)}</p></div>
			...
			<div className="h-2 overflow-hidden rounded-full bg-border" role="progressbar" aria-label={t("planUsage.usedAria", { name: label })} aria-valuemin={0} aria-valuemax={100} aria-valuenow={used ?? undefined}>
...
function QuotaHistory({ points }: { points: QuotaHistoryPoint[] }) {
	...
	return <div className="border-t border-border pt-3"><p className="mb-2 text-xs font-medium text-muted-foreground">{t("planUsage.history")}</p><svg aria-label={t("planUsage.historyAria")} className="h-16 w-full overflow-visible" role="img" viewBox={`0 0 ${width} ${height}`}><path d={path} fill="none" stroke="currentColor" strokeWidth="2" className="text-logo-accent" vectorEffect="non-scaling-stroke" /></svg></div>;
}
...
function PlanUsageSkeleton() { const { t } = useTranslation(); return <div className="grid gap-4 lg:grid-cols-2" aria-label={t("planUsage.loading")}><div className="h-72 animate-pulse rounded-lg bg-muted" /><div className="h-72 animate-pulse rounded-lg bg-muted" /></div>; }
