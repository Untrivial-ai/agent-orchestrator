	...
	return (
		<div className="border-t border-border py-1.5" data-testid="cloud-notification-list">
			<div className="flex items-center justify-between px-4 py-1.5">
				<span className="text-caption font-medium text-muted-foreground">{t("notify.cloud")}</span>
				<button className="text-caption text-muted-foreground hover:text-foreground" onClick={() => void markAllRead()} type="button">{t("notify.markCloudRead")}</button>
			</div>
			{items.map((notification) => (
				<div className="flex items-start gap-3 px-4 py-2.5" data-notification-source="cloud" key={notification.id} role="listitem">
					...
					<div className="min-w-0 flex-1">
						<div className="flex items-center gap-2"><span className="truncate text-control text-foreground">{notification.title}</span><span className="rounded-full border border-blue-400/25 bg-blue-400/10 px-1.5 py-0.5 text-[9px] font-medium text-blue-300">{t("notify.cloud")}</span></div>
