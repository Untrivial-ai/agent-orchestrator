...
export function CloudFileContentPane({
	...
	const tabs = <div className="sticky top-0 z-20 flex min-h-9 items-center gap-2 border-b border-border bg-surface px-2 py-1">
		...
		{annotation.target?.surface !== "review" && annotation.target?.path === path && annotation.target.side === "file" ? <div className="absolute right-2 top-full z-50 w-[min(32rem,calc(100%-1rem))]"><FileAnnotationComposer annotation={annotation} /></div> : null}
	</div>;
