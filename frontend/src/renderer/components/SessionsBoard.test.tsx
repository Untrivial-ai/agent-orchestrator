...
describe("SessionsBoard", () => {
	...
	it("shows token counts with coverage-aware cost details on active and archived cards", async () => {
		workspaceQueryMock.mockReturnValue({
			data: [
				workspaceWithSessions([
		...
		// The card shows tokens by default; cost remains in the hover tooltip and
		// accessible label.
		const activeUsage = screen.getByText("12.3K", { selector: "span" });
		expect(activeUsage).toHaveAttribute("aria-hidden", "true");
		expect(screen.getByText("12,300 tokens · $1.24")).toHaveClass("sr-only");
		...
		expect(within(archive).getByText("1,900 tokens · $0.02")).toHaveClass("sr-only");
	});

	it("shows tokens by default and tokens plus cost on hover without a tab stop", async () => {
		workspaceQueryMock.mockReturnValue({
			data: [
				workspaceWithSessions([
		...
		expect(within(card).getByText("12,400 tokens · $1.24")).toHaveClass("sr-only");
		...
		expect(await screen.findByRole("tooltip")).toHaveTextContent("12,400 tokens · $1.24");
	});
