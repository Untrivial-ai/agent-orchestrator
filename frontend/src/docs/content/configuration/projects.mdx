...
You can configure these paths from the project's **Settings → Workflow → Worktree setup** section. Add each repository-relative path explicitly; AO never links `.env` files implicitly. Because linked files can expose local secrets to worker sessions, only add paths you trust the worker to access.
...
Built-in reviewer harnesses are `aider`, `agy`, `amp`, `auggie`, `autohand`, `claude-code`, `cline`, `codex`, `copilot`, `crush`, `cursor`, `devin`, `droid`, `grok`, `kilocode`, `kimchi`, `kiro`, `kimi`, `opencode`, and `pi`. Kilo Code requires the
...
See [Reactions](/configuration/reactions) for the event list and action behavior.
...
<Cards>
	<Card title="Per-role agents" href="/guides/per-role-agents" description="Choose worker and orchestrator harnesses." />
	<Card title="Parallel work" href="/guides/parallel-issues" description="Start isolated sessions across projects." />
</Cards>
