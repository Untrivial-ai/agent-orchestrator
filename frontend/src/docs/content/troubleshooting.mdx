...
<Accordions>
	...
	<Accordion title="The QR code was shared">
		The QR carries the host, port, connection password, and any secure transport mode needed for pairing. Treat it as a bearer credential: do not share the QR or screenshots of it. Rotate the connection password in desktop settings if it may have been exposed.
	</Accordion>
...
| Symptom | Where to go |
| --- | --- |
| macOS Gatekeeper says the developer cannot be verified | Follow the Gatekeeper steps in [Common Installation Problems](/installation#common-installation-problems). |
