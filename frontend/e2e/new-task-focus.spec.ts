...
async function setup(page: Page, { animated = false } = {}) {
  ...
  await page.route("http://127.0.0.1:8080/api/v1/**", async (route) => {
    ...
    if (
      pathname === "/api/v1/agents/readiness" ||
      pathname === "/api/v1/agents/readiness/ensure"
    ) {
      await route.fulfill({
...
function openProjectMenu(page: Page) {
  return page
    .getByRole("button", {
      name: new RegExp(`Project actions for ${projectId}`),
    })
    .first()
    .click({ force: true });
}
...
async function expectSessionComposerReady(page: Page) {
  const composer = page.getByRole("combobox", { name: "Message the agent" });
  await expect(composer).toBeVisible({ timeout: 15_000 });
  return composer;
}

test("renderer: the board New task button focuses the composer prompt @T0", async ({
  page,
}) => {
  await setup(page);
...
for (const animated of [false, true]) {
  ...
  test(`renderer: New task from the sidebar project menu focuses the composer prompt (${motion}) @T0`, async ({
    ...
    await expectSessionComposerReady(page);
...
test("renderer: New task from the sidebar project context menu focuses the composer prompt @T0", async ({
  page,
}) => {
  await setup(page);
  await page.goto(`/#/projects/${projectId}/sessions/${sessionA}`);
...
test("renderer: New task from the command palette focuses the composer prompt @T0", async ({
  page,
}) => {
  await setup(page);
  await page.goto(`/#/projects/${projectId}/sessions/${sessionA}`);
...
test("renderer: opening another session focuses its chat composer @T0", async ({
  page,
}) => {
  await setup(page);
  await page.goto(`/#/projects/${projectId}/sessions/${sessionA}`);
  const composer = await expectSessionComposerReady(page);
  await expect
    .poll(async () => (await activeElementInfo(page)).label)
    .toBe("Message the agent");

  await page
    .getByRole("button", { name: /Open Session B/ })
    .first()
    .click();
...
async function setupSwitchAgentSession(page: Page) {
  ...
  await installFakeAgent(page, {
    ...
    projectName: projectId,
    workers: [
      { id: "switch-worker", provider: "claude-code", title: "Switch worker" },
    ],
  });
  await page.route("http://127.0.0.1:8080/api/v1/**", async (route) => {
    const pathname = new URL(route.request().url()).pathname;
...
test("renderer: a dialog opened from a session menu keeps focus inside itself @T0", async ({
  page,
}) => {
  ...
  await expect
    .poll(async () => (await activeElementInfo(page)).inDialog)
    .toBe(true);
  await expect(dialog).toBeVisible();
});

test("renderer: closing that dialog never strands focus on the page body @T0", async ({
  page,
}) => {
  await setupSwitchAgentSession(page);
  const dialog = await openSwitchAgentDialog(page);
...
test("renderer: a context-menu dialog returns focus to where the menu opened from @T0", async ({
  ...
  await page
    .getByRole("button", { name: new RegExp(openedFrom) })
    .first()
    .click({ button: "right", force: true });
  await page.getByRole("menuitem", { name: /New task/ }).click();
  await expect(page.getByRole("dialog")).toBeVisible();
