...
const longReplay = [
  ...
  ...Array.from(
    { length: 320 },
    (_, index) => `A retained history ${String(index).padStart(3, "0")}`,
  ),
].join("\r\n");
...
function xtermHost(container: Locator): Locator {
  return container.locator(".terminal-xterm-host");
}

function activeBufferAtBottom(page: Page): Promise<boolean> {
  return xtermHost(activeTerminal(page)).evaluate((element) => {
    const terminal = (element as HTMLElement & { __aoXtermForTest?: TestXterm })
      .__aoXtermForTest;
    return Boolean(
      terminal &&
...
async function observeNextReveal(container: Locator): Promise<void> {
  await container.evaluate((element) => {
    ...
    const captureVisibleSample = () => {
      const host = element.querySelector<HTMLElement>(".terminal-xterm-host");
      const viewport = host?.querySelector<HTMLElement>(".xterm-viewport");
      const terminal = (
        host as (HTMLElement & { __aoXtermForTest?: TestXterm }) | null
      )?.__aoXtermForTest;
      if (
        (element as HTMLElement).style.visibility === "hidden" ||
        (element as HTMLElement).dataset.terminalActivationPhase !==
          "visible" ||
        !viewport ||
        !terminal
      ) {
...
async function revealSamples(page: Page): Promise<RevealSample[]> {
  return page.evaluate(
    () => (window as RevealObserverWindow).__aoRevealSamples ?? [],
  );
}

async function revealFramePhases(page: Page): Promise<string[]> {
  return page.evaluate(
    () => (window as RevealObserverWindow).__aoRevealFrames ?? [],
  );
}

async function settledRevealSamples(page: Page): Promise<RevealSample[]> {
  await expect
    .poll(async () => (await revealSamples(page)).length)
    .toBeGreaterThan(0);
...
test.describe("retained terminal viewport", () => {
  test("shows and drags a slim macOS scrollbar for normal-buffer history", async ({
    page,
  }) => {
    ...
    const metrics = await host.evaluate((element) => {
      ...
      const terminal = (
        element as HTMLElement & { __aoXtermForTest?: TestXterm }
      ).__aoXtermForTest!;
    ...
    await page.evaluate(
      (handleId) =>
        window.__aoFakeTerminalMux!.emit(handleId, "\x1b[?1049hTUI screen"),
      handleA,
    );
    await expect(scrollbar).toHaveAttribute("data-scrollable", "false");
    await page.evaluate(
      (handleId) => window.__aoFakeTerminalMux!.emit(handleId, "\x1b[?1049l"),
      handleA,
    );
    await expect(scrollbar).toHaveAttribute("data-scrollable", "true");
  });
  ...
  test("retains the first of six live sessions with zero reopen and reveals its latest output", async ({
    ...
    const parkedA = page.locator(
      `[data-terminal-cache-key^="session:${sessionA.id}:worker|"]`,
    );
    await expect(parkedA).toHaveAttribute(
      "data-terminal-activation-phase",
      "parked",
    );
    await page.evaluate(
      (handleId) =>
        window.__aoFakeTerminalMux!.emit(
          ...
          "\r\n" +
            Array.from(
              { length: 20 },
              (_, index) => `A six-session hidden ${index}`,
            ).join("\r\n"),
        ),
      handleA,
    );
    ...
    await expect(
      activeTerminal(page).locator("[aria-label='Session terminal']"),
    ).toHaveAttribute("data-six-session-instance", "a");
    ...
    await expect
      .poll(() =>
        activeViewport(page).evaluate((element) => {
    ...
    expect(firstRevealFrames).toContain("visible");
    ...
    const aResizesBeforeReturn =
      beforeParkedGridChange.resizes[handleA]?.length ?? 0;
    const bResizesBeforeGridChange =
      beforeParkedGridChange.resizes[handleB]?.length ?? 0;
    ...
    const fontSizeBefore = await activeTerminal(page)
      ...
      .evaluate(
        (element) =>
          (element as HTMLElement & { __aoXtermForTest?: TestXterm })
            .__aoXtermForTest!.options.fontSize,
      );
    ...
    const activationResizePhases = (await muxStats(page)).resizePhases[
      handleA
    ]?.slice(aResizesBeforeReturn);
    ...
    // Hidden output can leave the offscreen DOM scroll position stale. A
    // retained terminal may reveal immediately, but it must settle at the
    // canonical buffer tail without reopening its attachment.
    ...
    await observeNextReveal(parkedA);
    await openSession(page, sessionA.title);
    await settledRevealSamples(page);
    await expect.poll(() => activeBufferAtBottom(page)).toBe(true);
    await expect
      .poll(() =>
        activeViewport(page).evaluate((element) => {
  ...
  test("suppresses parked resize and scroll storms during rapid A-B-A switching", async ({
    ...
    await viewport.evaluate((element) => {
      (
        element as HTMLElement & { __aoScrollEvents?: number }
      ).__aoScrollEvents = 0;
    ...
    expect(
      await parkedA.evaluate((element) => (element as HTMLElement).inert),
    ).toBe(true);
    ...
    await page.evaluate((handleId) => {
      window.__aoFakeTerminalMux!.emit(
        ...
        "\r\n" +
          Array.from(
            { length: 30 },
            (_, index) => `A hidden output ${index}`,
          ).join("\r\n"),
      );
    }, handleA);
    const aResizesBeforeReturn =
      (await muxStats(page)).resizes[handleA]?.length ?? 0;
    ...
    expect(
      afterReturn.resizePhases[handleA]?.slice(aResizesBeforeReturn) ?? [],
    ).toEqual([]);
    await page.evaluate((handleId) => {
    ...
    await expect(
      page.locator("[data-terminal-cache-key]:not([aria-hidden='true'])"),
    ).toHaveCount(1);
  });

  test("keeps stable attachments across warmed orchestrator-worker switches", async ({
    page,
  }) => {
    await installHarness(page);
    ...
    expect(after.resizes[handleA]?.length ?? 0).toBeGreaterThanOrEqual(
      workerResizes,
    );
    expect(
      after.resizes[orchestratorHandle]?.length ?? 0,
    ).toBeGreaterThanOrEqual(orchestratorResizes);
    expect(after.resizePhases[handleA]?.slice(workerResizes)).not.toContain(
      "parked",
    );
    expect(
      after.resizePhases[orchestratorHandle]?.slice(orchestratorResizes),
    ).not.toContain("parked");
    expect(after.opens[handleA]).toBe(1);
    expect(after.opens[orchestratorHandle]).toBe(1);
  });

  test("handles empty and short replay, blocks hidden input, and retains xterm through reconnect", async ({
    page,
  }) => {
    await installHarness(page);
    const originalViewport = activeViewport(page);
    await originalViewport.evaluate((element) =>
      element.setAttribute("data-reconnect-instance", "a"),
    );
    await activeTerminal(page).locator(".xterm-screen").hover();
    await page.mouse.wheel(0, -1_400);
    ...
    await expect
      ...
      .toContain("b-input");
    expect((await muxStats(page)).inputs[handleA]?.length ?? 0).toBe(
      inputsBefore,
    );
    ...
    await page.evaluate(
      (handleId) => window.__aoFakeTerminalMux!.disconnect(handleId),
      handleA,
    );
    await expect(
      activeTerminal(page).getByText("Terminal disconnected — reattaching…"),
    ).toBeVisible();
    await expect
      .poll(async () => (await muxStats(page)).opens[handleA] ?? 0)
      .toBe(2);
    await expect
      .poll(async () => (await muxStats(page)).opens[handleB] ?? 0)
      .toBe(2);
    await expect
      .poll(async () => (await muxStats(page)).opens[handleC] ?? 0)
      .toBe(2);
    ...
    expect(
      returnStats.resizes[handleA][returnResizeIndex].cols,
    ).toBeGreaterThan(0);
    expect(
      returnStats.resizes[handleA][returnResizeIndex].rows,
    ).toBeGreaterThan(0);
    expect(returnStats.resizePhases[handleA][returnResizeIndex]).toBe(
      "visible",
    );
    await expect(activeViewport(page)).toHaveAttribute(
      "data-reconnect-instance",
      "a",
    );
    await expect(page.getByTestId("terminal-replay-cover")).toHaveCount(0);
    await expect(activeViewport(page)).toHaveAttribute(
    ...
    await expect.poll(() => activeBufferAtBottom(page)).toBe(true);
    expect(retainedSelection).toBe("A retained history 124");
    expect(afterReconnectSelection).toBe("");
    ...
    await page.evaluate((handleId) => {
      window.__aoFakeTerminalMux!.emit(
        ...
        "\r\n" +
          Array.from(
            { length: 5_300 },
            (_, index) => `A eviction output ${String(index).padStart(4, "0")}`,
          ).join("\r\n"),
      );
    }, handleA);
    await expect
      .poll(() =>
        parkedA.locator(".terminal-xterm-host").evaluate((element) => {
          const terminal = (
            element as HTMLElement & { __aoXtermForTest?: TestXterm }
          ).__aoXtermForTest!;
