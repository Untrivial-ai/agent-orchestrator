...
describe("CloudClient", () => {
	it("loads the authenticated account and organization memberships", async () => {
		...
		const fetchMock = vi.fn(async (_input: RequestInfo | URL, _init?: RequestInit) => jsonResponse(account));
		...
		expect(fetchMock.mock.calls[0]?.[0]).toBe("https://cloud.example.com/api/cloud/v1/me");
	});
	...
	it("requests durable deletion on the session resource", async () => {
		...
		await client.deleteSession("4165753c-c6ad-4ac2-8f12-e0cbb24d9750", "a9dc6493-bd04-4c03-bb45-55733ed83784");
	...
	it("updates editable project settings on the project resource", async () => {
		...
		const fetchMock = vi.fn(async (_input: RequestInfo | URL, _init?: RequestInit) => jsonResponse({ project }));
		const client = createCloudClient({
			baseUrl: "https://cloud.example.com",
			getAccessToken: () => "access-token",
			fetch: fetchMock as typeof fetch,
	...
	it("requests durable project deletion on the project resource", async () => {
		...
		await expect(client.deleteProject("org one", "project one")).resolves.toEqual({
	...
	it("lists runtime-supplied agent profiles for an organization", async () => {
		...
		await expect(client.listAgents("tenant one/blue")).resolves.toEqual([profile]);
		expect(fetchMock.mock.calls[0]?.[0]).toBe("https://cloud.example.com/api/cloud/v1/orgs/tenant%20one%2Fblue/agents");
	});
	...
	it("manages organization-scoped GitHub App installations and repositories", async () => {
		...
		await expect(client.listGitHubInstallations("tenant one")).resolves.toEqual([installation]);
		await client.startGitHubInstallation("tenant one");
		await client.listGitHubRepositories("tenant one", {
			cursor: "next page",
			limit: 25,
		});
		await client.syncGitHubInstallation("tenant one", "d9916dbe-486c-43ec-91b8-379419767719");
		await client.disconnectGitHubInstallation("tenant one", "d9916dbe-486c-43ec-91b8-379419767719");
		...
		expect(requestHeaders(fetchMock, 5).get("Idempotency-Key")).toBe("github-project-1");
	});
	...
	it("manages user GitHub authorization and scratch project creation", async () => {
		...
		expect(requestHeaders(fetchMock, 2).get("Idempotency-Key")).toBe("scratch-project-1");
		expect(fetchMock.mock.calls[3]?.[1]?.method).toBe("DELETE");
	});

	it("scopes and encodes organization and session URLs", async () => {
		const fetchMock = vi.fn(async (_input: RequestInfo | URL, _init?: RequestInit) => jsonResponse({ session: {} }));
		const client = createCloudClient({
			baseUrl: "https://cloud.example.com/",
			getAccessToken: () => "access-token",
			fetch: fetchMock as typeof fetch,
	...
	it("writes a workspace file through the typed client", async () => {
		...
		const fetchMock = vi.fn(async (_input: RequestInfo | URL, _init?: RequestInit) => jsonResponse(file));
		const client = createCloudClient({
			baseUrl: "https://cloud.example.com",
			getAccessToken: () => "access-token",
			fetch: fetchMock as typeof fetch,
	...
	it("reads normalized pull requests and AO review state for a session", async () => {
		...
		await expect(client.listSessionPullRequests("tenant", "session one?")).resolves.toEqual({
			sessionId: "session one?",
			pullRequests: [pullRequest],
		});
		await expect(client.getSessionReviewState("tenant", "session one?")).resolves.toEqual(reviewState);
		expect(fetchMock.mock.calls.map((call) => call[0])).toEqual([
	...
	it("injects the latest access token into every request", async () => {
		...
		expect(requestHeaders(fetchMock, 0).get("Authorization")).toBe("Bearer first-token");
		expect(requestHeaders(fetchMock, 1).get("Authorization")).toBe("Bearer second-token");
	});
	...
	it("connects and disconnects coding-agent credentials", async () => {
		...
		await expect(client.deleteAgentProviderConnection("tenant", "claude-code")).resolves.toBeUndefined();
	...
	it("passes the replay cursor and page limit", async () => {
		...
		await expect(client.replayEvents("tenant", "session", { after: 41, limit: 25 })).resolves.toEqual({
			events: [event],
			hasMore: true,
			nextAfter: 42,
		});
	...
	it("sends idempotency keys on mutating commands", async () => {
		...
		expect(requestHeaders(fetchMock, 0).get("Idempotency-Key")).toBe("message-command-1");
		expect(fetchMock.mock.calls[0]?.[1]).toMatchObject({
			method: "POST",
	...
	it("streams replayed SSE events from an explicit cursor", async () => {
		...
		expect(requestHeaders(fetchMock, 0).get("Accept")).toBe("text/event-stream");
	});
	...
	it("reconnects event streams from the greatest consumed sequence", async () => {
		...
		const getAccessToken = vi.fn<() => string>().mockReturnValueOnce("first-token").mockReturnValueOnce("second-token");
		const client = createCloudClient({
			baseUrl: "https://cloud.example.com",
			getAccessToken,
			fetch: fetchMock as typeof fetch,
	...
	it("durably requests turn cancellation with an idempotency key", async () => {
		const fetchMock = vi.fn<(input: RequestInfo | URL, init?: RequestInit) => Promise<Response>>(async () =>
			jsonResponse({ turn: { id: "turn-1" } }, 202),
		);
		const client = createCloudClient({
			baseUrl: "https://cloud.example.com",
			getAccessToken: () => "access-token",
		...
		expect(requestHeaders(fetchMock, 0).get("Idempotency-Key")).toBe("cancel-turn-1");
	});
});
...
describe("WorkerClient", () => {
	it("matches the worker lifecycle, execution, orchestration, and transport routes", async () => {
		...
		const fetchMock = vi
			...
			.mockResolvedValueOnce(jsonResponse({ session: { id: "child", desiredState: "deleted" } }, 202))
		...
		for (let index = 1; index < fetchMock.mock.calls.length; index += 1) {
			expect(requestHeaders(fetchMock, index).get("Authorization")).toBe("Worker worker-token");
		}
		...
		expect(requestHeaders(fetchMock, 11).get("Idempotency-Key")).toBe("message-1");
