...
-- Recovery reconciliation asks a replacement worker to prove that the
-- committed controller is live before releasing prompts held by a failed
-- interface handoff.
...
DELETE FROM ao_worker_requests WHERE kind = 'interface.ready';
ALTER TABLE ao_worker_requests
    DROP CONSTRAINT ao_worker_requests_kind_check;
ALTER TABLE ao_worker_requests
    ADD CONSTRAINT ao_worker_requests_kind_check
